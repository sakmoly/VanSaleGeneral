...
public class DatabaseHandler extends SQLiteOpenHelper {
   ...
   // private static final String LAST_DOC_NO_CUSTOMER_VISIT = "last_doc_no_customer_visit";
    ...
    private static final String CUSTOMER_VISIT_ACCESS="customer_visit_access";
    private static final String MANAGE_FREEZER="manage_freezer";
...
/* ========================== Sales Invoice Payment ========================= */
    ...
    try {
        ...
        String CREATE_SETTINGS_TABLE = "CREATE TABLE " + TABLE_SETTINGS + "(" +
                ...
                CUSTOMER_VISIT_ACCESS + " INTEGER," +
                MANAGE_FREEZER + " INTEGER," +
                TRANS_DEFAULT_QTY + " INTEGER," +
                LAST_SYNC_TIME + " TEXT" +
                ")";
    ...
    public void onUpgrade(SQLiteDatabase db, int oldversion, int i1) {
        ...
        if(oldversion<6){
            db.execSQL("ALTER TABLE "+TABLE_SETTINGS+ " ADD COLUMN "+CUSTOMER_VISIT_ACCESS+" INTEGER"+" DEFAULT 0");
            db.execSQL("ALTER TABLE "+TABLE_SETTINGS+ " ADD COLUMN "+MANAGE_FREEZER+" INTEGER"+" DEFAULT 0");
        }
        if(oldversion<7){
    ...
    public void addSettings(SettingsClass settingsClass) {
        ...
        values.put(CUSTOMER_VISIT_ACCESS,settingsClass.getCustomer_visit_access());
        values.put(MANAGE_FREEZER,settingsClass.getManage_freezer());
    ...
    public List<Payment> getPaymentList() {
        ...
        if (cursor.moveToFirst()) {
            do {
                ...
                orderClass.setPAYMENT_REFERENCE_DATE(cursor.getString(8));
                orderClass.setPAYMENT_PAID_TO(cursor.getString(9));
                orderClass.setPAYMENT_OWNER(cursor.getString(10));
                arrayList.add(orderClass);
   ...
   /* private String getSalesOrderCustomerCode(String string) {
    ...
    public List<CustomerClass> getCustomer1() {
        List<CustomerClass> arrayList = new ArrayList<CustomerClass>();

        //String selectQuery = "SELECT  * FROM " + TABLE_CUSTOMER+ " ORDER BY "+SERVER_CUSTOMER_ID+" ASC ";
        String selectQuery = "SELECT  C.* FROM " + TABLE_CUSTOMER+
                " AS C ORDER BY C."+SERVER_CUSTOMER_ID+" ASC ";
        SQLiteDatabase db = this.getWritableDatabase();
        Cursor cursor = db.rawQuery(selectQuery, null);
    ...
    try {
            ...
            if (cursor.moveToFirst()) {
                do {
                    ...
                    settingsClass.setCustomer_visit_access(cursor.getInt(34));
                    settingsClass.setManage_freezer(cursor.getInt(35));

                    if(cursor.getString(36)!=null)
                        settingsClass.setDefault_trans_qty(cursor.getFloat(36));
                    else
